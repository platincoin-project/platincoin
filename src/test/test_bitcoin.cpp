...
#define BOOST_TEST_MODULE Platincoin Test Suite
...
TestingSetup::TestingSetup(const std::string& chainName) : BasicTestingSetup(chainName)
{
    const CChainParams& chainparams = Params();
        ...
        ClearDatadirCache();
        pathTemp = GetTempPath() / strprintf("test_platincoin_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(100000)));
...
CTxMemPoolEntry TestMemPoolEntryHelper::FromTx(const CTransaction &txn, CTxMemPool *pool) {
    ...
    return CTxMemPoolEntry(MakeTransactionRef(txn), nFee, 0, nTime, dPriority, nHeight,
