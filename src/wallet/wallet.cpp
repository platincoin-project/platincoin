...
bool CReserveKey::GetReservedKey(CPubKey & pubkey)
{
    ...
    CKeyPool keypool;
    m_pwallet->ReserveKeyFromKeyPool(index, keypool);
    if (index == -1)
    {
        return false;
    }
    ...
    assert(keypool.vchPubKey.IsValid());
    m_keys.emplace_back(index, keypool.vchPubKey);
...
void CReserveKey::KeepKeys()
{
    for (const WalletKey & key : m_keys)
    {
        if (key.index != -1)
        {
            m_pwallet->KeepKey(key.index);
        }
    }
    m_keys.clear();
}

void CReserveKey::ReturnKeys()
{
    for (const WalletKey & key : m_keys)
    {
        if (key.index != -1)
        {
            m_pwallet->ReturnKey(key.index);
        }
    }
    m_keys.clear();
}
...
void CWallet::GetScriptForMining(boost::shared_ptr<CReserveScript> &script)
{
    ...
    script = rKey;
    script->subsidyDestination = GetScriptForRawPubKey(pubkey);
