...
const char * const BITCOIN_CONF_FILENAME = "platincoin.conf";
const char * const BITCOIN_PID_FILENAME = "platincoin.pid";
...
#else
    const char* pszModule = "platincoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Platincoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Platincoin";
#else
    // Unix
    return pathRet / ".platincoin";
#endif
#endif
}
