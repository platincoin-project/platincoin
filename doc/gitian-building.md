...
*Setup instructions for a Gitian build of Platincoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Platincoin
...
- [Building Platincoin Core](#building-platincoin-core)
...
Clone the git repositories for platincoin and Gitian.
...
in the platincoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Platincoin Core
----------------

To build Platincoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the platincoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/platincoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/platincoin-project/platincoin
...
For example:
```bash
URL=https://github.com/thrasher-/platincoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit platincoin=${COMMIT} --url platincoin=${URL} ../platincoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit platincoin=${COMMIT} --url platincoin=${URL} ../platincoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit platincoin=${COMMIT} --url platincoin=${URL} ../platincoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the platincoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../platincoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/platincoin
SIGPATH=/some/root/path/platincoin-detached-sigs

./bin/gbuild --url platincoin=${BTCPATH},signature=${SIGPATH} ../platincoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/platincoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/platincoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/platincoin-osx-build.assert
```
...
[platincoin-project/gitian.sigs.ltc](https://github.com/platincoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
