...
class WalletAccountsTest(BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        assert_equal(node.getbalance(), 6000000*10)
        ...
        expected_balance = 6000000 * 100 + Decimal('0.005') * 11 - fees1 - fees2 + max(fees1 / 2, Decimal('0.005')) + max(fees2 / 2, Decimal('0.005'))
        expected_account_balances = { "": expected_balance }
        for account in accounts:
            expected_account_balances[account] = 0

        assert_equal(node.getbalance(""), expected_balance)
