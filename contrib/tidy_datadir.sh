...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Platincoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Platincoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Platincoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Platincoin 0.7 datadir."
    ;;
  3)
    echo "Detected Platincoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Platincoin 0.8 datadir."
    ;;
esac
